...
coverage = {
    ...
    **({"li_join_gaps_note": (
    ...
    "li_source_note": (
        ...
        "SHARES CONTACTS WITH A COLLEAGUE. Re-measured contact by contact on 5 Aug 2026 against the "
        "connected Groovin account, which is Elliott's. That account held 16 pending sent "
        "invitations that LinkedIn dates to 4 Aug. Attio holds a 4 Aug invitation for only four of "
        "them, and credits two to Elliott, one to Luke (Gennetti) and one to James (Brost). Two "
        "causes, both structural. First, eight of the sixteen went to profiles Groovin does not pair "
        "to the CRM: they have an Attio person record but no invitation field at all, so the send "
        "was never written anywhere. Second, the record holds only the LAST invitation per contact "
        "and can name only one sender, so where two seats invite the same contact the earlier send "
        "is overwritten and one person loses the credit entirely. That is the most likely reading of "
        "Gennetti and Brost, whom Luke and James were working the same day. Two more (Smith, "
        "Ritchie) carry a 3 Aug stamp, and all sixteen pendings share one identical synthetic "
        "timestamp, so LinkedIn's pending date is reliable only to about the day. Chris's own "
        "Groovin panel showed 45 sent against 15 in Attio on 4 Aug, and his LinkedIn account cannot "
